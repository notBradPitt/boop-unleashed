...
def start_cam(stream_to_obs, use_xseg, use_mouthrestore, cam, reso, enhancer, blend_ratio, upscale):
    from boop.virtualcam import start_virtual_cam
    from boop.utilities import convert_to_gradio

    boop.globals.selected_enhancer = enhancer
    boop.globals.blend_ratio = blend_ratio
    boop.globals.subsample_size = int(upscale[:3])
...
def stop_swap():
    from boop.virtualcam import stop_virtual_cam
    stop_virtual_cam()
