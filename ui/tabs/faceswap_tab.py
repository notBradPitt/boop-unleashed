...
import pathlib
import gradio as gr
import boop.utilities as util
import boop.globals
import ui.globals
from boop.face_util import extract_face_images, create_blank_image
from boop.capturer import get_video_frame, get_video_frame_total, get_image_frame
from boop.ProcessEntry import ProcessEntry
from boop.ProcessOptions import ProcessOptions
from boop.FaceSet import FaceSet
...
def faceswap_tab():
    ...
    with gr.Tab("🎭 Face Swap"):
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=1):
                with gr.Group():
                    ...
                    boop.globals.skip_audio = gr.Checkbox(label="Skip audio", value=False)
                    boop.globals.keep_frames = gr.Checkbox(label="Keep Frames (relevant only when extracting frames)", value=False)
                    boop.globals.wait_after_extraction = gr.Checkbox(label="Wait for user key press before creating video ", value=False)
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column():
                ...
                gr.Button("👀 Open Output Folder", size='sm').click(fn=lambda: util.open_folder(boop.globals.output_path))
            with gr.Column(scale=2):
    ...
    start_event = bt_start.click(fn=start_swap, 
        inputs=[output_method, ui.globals.ui_selected_enhancer, selected_face_detection, boop.globals.keep_frames, boop.globals.wait_after_extraction,
                    boop.globals.skip_audio, max_face_distance, ui.globals.ui_blend_ratio, selected_mask_engine, clip_text,video_swapping_method, no_face_action, vr_mode, autorotate, chk_restoreoriginalmouth, num_swap_steps, ui.globals.ui_upscale, maskimage],
...
def set_mask_offset(index, mask_offset):
    ...
    if len(boop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        offs = boop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets
        ...
        if offs[2] + offs[3] > 0.99:
            offs[2] = 0.99
            offs[3] = 0.0
        boop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets = offs
    ...
    for f in srcfiles:    
        ...
        if source_path.lower().endswith('fsz'):
            ...
            if len(face_set.faces) > 0:
                ...
                boop.globals.INPUT_FACESETS.append(face_set)
                                        
        elif util.has_image_extension(source_path):
            progress(0, desc="Retrieving faces from image")      
            boop.globals.source_path = source_path
            SELECTION_FACES_DATA = extract_face_images(boop.globals.source_path,  (False, 0))
...
def remove_selected_input_face():
    ...
    if len(boop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        f = boop.globals.INPUT_FACESETS.pop(SELECTED_INPUT_FACE_INDEX)
        del f
...
def move_selected_input(button_text):
    ...
    f = boop.globals.INPUT_FACESETS.pop(SELECTED_INPUT_FACE_INDEX)
    boop.globals.INPUT_FACESETS.insert(SELECTED_INPUT_FACE_INDEX + offset, f)
    f = ui.globals.ui_input_thumbs.pop(SELECTED_INPUT_FACE_INDEX)
...
def move_selected_target(button_text):
    ...
    f = boop.globals.TARGET_FACES.pop(SELECTED_TARGET_FACE_INDEX)
    boop.globals.TARGET_FACES.insert(SELECTED_TARGET_FACE_INDEX + offset, f)
...
def on_use_face_from_selected(files, frame_num):
    ...
    boop.globals.target_path = files[selected_preview_index].name
    if util.is_image(boop.globals.target_path) and not boop.globals.target_path.lower().endswith(('gif')):
        SELECTION_FACES_DATA = extract_face_images(boop.globals.target_path,  (False, 0))
        if len(SELECTION_FACES_DATA) > 0:
            ...
            boop.globals.target_path = None
                
    elif util.is_video(boop.globals.target_path) or boop.globals.target_path.lower().endswith(('gif')):
        selected_frame = frame_num
        SELECTION_FACES_DATA = extract_face_images(boop.globals.target_path, (True, selected_frame))
        if len(SELECTION_FACES_DATA) > 0:
            for f in SELECTION_FACES_DATA:
                image = util.convert_to_gradio(f[1])
                thumbs.append(image)
    ...
    if len(thumbs) == 1:
        boop.globals.TARGET_FACES.append(SELECTION_FACES_DATA[0][0])
...
def on_selected_face():
    ...
    if IS_INPUT:
        ...
        boop.globals.TARGET_FACES.append(fd[0])
...
def on_preview_frame_changed(frame_num, files, fake_preview, enhancer, detection, face_distance, blend_ratio,
    ...
    if len(boop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        if not hasattr(boop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0], 'mask_offsets'):
            boop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets = mask_offsets
        mask_offsets = boop.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets
    ...
    if not fake_preview or len(boop.globals.INPUT_FACESETS) < 1:
    ...
    boop.globals.face_swap_mode = translate_swap_mode(detection)
    boop.globals.selected_enhancer = enhancer
    boop.globals.distance_threshold = face_distance
    boop.globals.blend_ratio = blend_ratio
    boop.globals.no_face_action = index_of_no_face_action(no_face_action)
    boop.globals.vr_mode = vr_mode
    boop.globals.autorotate_faces = auto_rotate
    boop.globals.subsample_size = int(upsample[:3])


    mask_engine = map_mask_engine(selected_mask_engine, clip_text)

    boop.globals.execution_threads = boop.globals.CFG.max_threads
    mask = layers[0] if layers is not None else None
    face_index = SELECTED_INPUT_FACE_INDEX
    if len(boop.globals.INPUT_FACESETS) <= face_index:
        face_index = 0
   
    options = ProcessOptions(get_processing_plugins(mask_engine), boop.globals.distance_threshold, boop.globals.blend_ratio,
                              boop.globals.face_swap_mode, face_index, clip_text, maskimage, num_steps, boop.globals.subsample_size, show_face_area, restore_original_mouth)
...
def start_swap( output_method, enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio,
    ...
    from boop.core import batch_process_regular
    global is_processing, list_files_process

    if list_files_process is None or len(list_files_process) <= 0:
        return gr.Button(variant="primary"), None, None
    
    if boop.globals.CFG.clear_output:
        clean_dir(boop.globals.output_path)
    ...
    boop.globals.distance_threshold = face_distance
    boop.globals.blend_ratio = blend_ratio
    boop.globals.keep_frames = keep_frames
    boop.globals.wait_after_extraction = wait_after_extraction
    boop.globals.skip_audio = skip_audio
    boop.globals.face_swap_mode = translate_swap_mode(detection)
    boop.globals.no_face_action = index_of_no_face_action(no_face_action)
    boop.globals.vr_mode = vr_mode
    boop.globals.autorotate_faces = autorotate
    boop.globals.subsample_size = int(upsample[:3])
    mask_engine = map_mask_engine(selected_mask_engine, clip_text)

    if boop.globals.face_swap_mode == 'selected':
        if len(boop.globals.TARGET_FACES) < 1:
    ...
    boop.globals.execution_threads = boop.globals.CFG.max_threads
    boop.globals.video_encoder = boop.globals.CFG.output_video_codec
    boop.globals.video_quality = boop.globals.CFG.video_quality
    boop.globals.max_memory = boop.globals.CFG.memory_limit if boop.globals.CFG.memory_limit > 0 else None
    ...
    is_processing = False
    outdir = pathlib.Path(boop.globals.output_path)
...
def get_gradio_output_format():
    if boop.globals.CFG.output_image_format == "jpg":
        return "jpeg"
    return boop.globals.CFG.output_image_format


def display_output(filename):
    if util.is_video(filename) and boop.globals.CFG.output_show_video:
