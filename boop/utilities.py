...
import boop.template_parser as template_parser
...
def sort_rename_frames(path: str):
    ...
    for i in range(len(filenames)):
        ...
        new_filename = os.path.join(
            path, f"{newidx:06d}." + boop.globals.CFG.output_image_format
        )
    ...
    return glob.glob(
        (
            os.path.join(
                ...
                f"*.{boop.globals.CFG.output_image_format}",
            )
        )
    )
...
def replace_template(file_path: str, index: int = 0) -> str:
    ...
    template = boop.globals.CFG.output_template
    replaced_filename = template_parser.parse(
        template, {"index": str(index), "file": fn}
    )

    return os.path.join(boop.globals.output_path, f"{replaced_filename}{ext}")
...
def clean_temp(target_path: str) -> None:
    ...
    if not boop.globals.keep_frames and os.path.isdir(temp_directory_path):
...
def get_device() -> str:
    if len(boop.globals.execution_providers) < 1:
        boop.globals.execution_providers = ["CPUExecutionProvider"]

    prov = boop.globals.execution_providers[0]
...
def prepare_for_batch(target_files) -> str:
    ...
    tempfolder = os.path.join(tempfile.gettempdir(), "booptmp")
