...
import boop.globals
import boop.metadata
import boop.utilities as util
import boop.util_ffmpeg as ffmpeg
import ui.main as main
from settings import Settings
from boop.face_util import extract_face_images
from boop.ProcessEntry import ProcessEntry
from boop.ProcessMgr import ProcessMgr
from boop.ProcessOptions import ProcessOptions
from boop.capturer import get_video_frame_total, release_video
...
if 'ROCMExecutionProvider' in boop.globals.execution_providers:
    del torch
...
def parse_args() -> None:
    ...
    boop.globals.startup_args = program.parse_args()
    # Always enable all processors when using GUI
    boop.globals.frame_processors = ['face_swapper', 'face_enhancer']
    ...
    try:
        for i in range(len(list_providers)):
            if list_providers[i] == 'CUDAExecutionProvider':
                list_providers[i] = ('CUDAExecutionProvider', {'device_id': boop.globals.cuda_device_id})
                torch.cuda.set_device(boop.globals.cuda_device_id)
                break
    except:
        pass
...
def suggest_execution_threads() -> int:
    if 'DmlExecutionProvider' in boop.globals.execution_providers:
        return 1
    if 'ROCMExecutionProvider' in boop.globals.execution_providers:
        return 1
    return 8
...
def limit_resources() -> None:
    # limit memory usage
    if boop.globals.max_memory:
        memory = boop.globals.max_memory * 1024 ** 3
        if platform.system().lower() == 'darwin':
            memory = boop.globals.max_memory * 1024 ** 6
...
def release_resources() -> None:
    ...
    gc.collect()
    # if 'CUDAExecutionProvider' in boop.globals.execution_providers and torch.cuda.is_available():
...
def start() -> None:
    if boop.globals.headless:
        print('Headless mode currently unsupported - starting UI!')
        # faces = extract_face_images(boop.globals.source_path,  (False, 0))
        # boop.globals.INPUT_FACES.append(faces[boop.globals.source_face_index])
        # faces = extract_face_images(boop.globals.target_path,  (False, util.has_image_extension(boop.globals.target_path)))
        # boop.globals.TARGET_FACES.append(faces[boop.globals.target_face_index])
        # if 'face_enhancer' in boop.globals.frame_processors:
        #     boop.globals.selected_enhancer = 'GFPGAN'
...
def get_processing_plugins(masking_engine):
    ...
    if boop.globals.selected_enhancer == 'GFPGAN':
        processors.update({"gfpgan": {}})
    elif boop.globals.selected_enhancer == 'Codeformer':
        processors.update({"codeformer": {}})
    elif boop.globals.selected_enhancer == 'DMDNet':
        processors.update({"dmdnet": {}})
    elif boop.globals.selected_enhancer == 'GPEN':
        processors.update({"gpen": {}})
    elif boop.globals.selected_enhancer == 'Restoreformer++':
...
#    if len(boop.globals.INPUT_FACESETS) <= selected_index:
#        selected_index = 0
    process_mgr.initialize(boop.globals.INPUT_FACESETS, boop.globals.TARGET_FACES, options)
...
def batch_process_regular(output_method, files:list[ProcessEntry], masking_engine:str, new_clip_text:str, use_new_method, imagemask, restore_original_mouth, num_swap_steps, progress, selected_index = 0) -> None:
    ...
    if len(boop.globals.INPUT_FACESETS) <= selected_index:
        selected_index = 0
    options = ProcessOptions(get_processing_plugins(masking_engine), boop.globals.distance_threshold, boop.globals.blend_ratio,
                              boop.globals.face_swap_mode, selected_index, new_clip_text, mask, num_swap_steps,
                              boop.globals.subsample_size, False, restore_original_mouth)
    process_mgr.initialize(boop.globals.INPUT_FACESETS, boop.globals.TARGET_FACES, options)
...
def batch_process_with_options(files:list[ProcessEntry], options, progress):
    ...
    if process_mgr is None:
        process_mgr = ProcessMgr(progress)
    process_mgr.initialize(boop.globals.INPUT_FACESETS, boop.globals.TARGET_FACES, options)
    boop.globals.keep_frames = False
    boop.globals.wait_after_extraction = False
    boop.globals.skip_audio = False
...
def batch_process(output_method, files:list[ProcessEntry], use_new_method) -> None:
    ...
    for index, f in enumerate(files):
        ...
        if util.has_image_extension(fullname):
            destination = util.get_destfilename_from_path(fullname, boop.globals.output_path, f'.{boop.globals.CFG.output_image_format}')
            ...
            destination = util.get_destfilename_from_path(fullname, boop.globals.output_path, f'__temp.{boop.globals.CFG.output_video_format}')
            f.finalname = destination
            videofiles.append(f)
    ...
    if(len(imagefiles) > 0):
        ...
        process_mgr.run_batch(origimages, fakeimages, boop.globals.execution_threads)
        origimages.clear()
        fakeimages.clear()

    if(len(videofiles) > 0):
        for index,v in enumerate(videofiles):
            if not boop.globals.processing:
            ...
            if is_streaming_only == False and boop.globals.keep_frames or not use_new_method:
                ...
                process_mgr.run_batch(temp_frame_paths, temp_frame_paths, boop.globals.execution_threads)
                if not boop.globals.processing:
                    end_processing('Processing stopped!')
                    return
                if boop.globals.wait_after_extraction:
                ...
                if not boop.globals.keep_frames:
                    util.delete_temp_frames(temp_frame_paths[0])
            else:
                if util.has_extension(v.filename, ['gif']):
                    skip_audio = True
                else:
                    skip_audio = boop.globals.skip_audio
                process_mgr.run_batch_inmem(output_method, v.filename, v.finalname, v.startframe, v.endframe, fps,boop.globals.execution_threads, skip_audio)
            ...
            if os.path.isfile(video_file_name):
                destination = ''
                if util.has_extension(v.filename, ['gif']):
                    gifname = util.get_destfilename_from_path(v.filename, boop.globals.output_path, '.gif')
...
def end_processing(msg:str):
    update_status(msg)
    boop.globals.target_folder_path = None
    release_resources()


def destroy() -> None:
    if boop.globals.target_path:
        util.clean_temp(boop.globals.target_path)
    release_resources()        
    sys.exit()
...
def run() -> None:
    ...
    if not pre_check():
        return
    boop.globals.CFG = Settings('config.yaml')
    boop.globals.cuda_device_id = boop.globals.startup_args.cuda_device_id
    ...
    if boop.globals.startup_args.server_share:
        boop.globals.CFG.server_share = True
    main.run()
