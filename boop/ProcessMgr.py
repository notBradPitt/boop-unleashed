...
from boop.face_util import get_first_face, get_all_faces, rotate_anticlockwise, rotate_clockwise, clamp_cut_values
from boop.utilities import compute_cosine_distance, get_device, str_to_class
import boop.vr_util as vr

from typing import Any, List, Callable
from boop.typing import Frame, Face
from concurrent.futures import ThreadPoolExecutor, as_completed
from threading import Thread, Lock
from queue import Queue
from tqdm import tqdm
from boop.ffmpeg_writer import FFMPEG_VideoWriter
from boop.StreamWriter import StreamWriter
import boop.globals
...
class ProcessMgr():
    ...
    def initialize(self, input_faces, target_faces, options):
        ...
        boop.globals.g_desired_face_analysis=["landmark_3d_68", "landmark_2d_106","detection","recognition"]
        if options.swap_mode == "all_female" or options.swap_mode == "all_male":
            boop.globals.g_desired_face_analysis.append("genderage")
        ...
        for key, extoption in options.processors.items():
            ...
            if p is None:
                ...
                module = 'boop.processors.' + classname
    ...
    def read_frames_thread(self, cap, frame_start, frame_end, num_threads):
        ...
        while True and boop.globals.processing:
            ret, frame = cap.read()
            if not ret:
                break
    ...
    def run_batch_inmem(self, output_method, source_video, target_video, frame_start, frame_end, fps, threads:int = 1, skip_audio=False):
        ...
        if self.output_to_file:
            self.videowriter = FFMPEG_VideoWriter(target_video, (width, height), fps, codec=boop.globals.video_encoder, crf=boop.globals.video_quality, audiofile=None)
        if self.output_to_cam:
    ...
    def process_frame(self, frame:Frame):
        ...
        if num_swapped > 0:
            if boop.globals.no_face_action == eNoFaceAction.SKIP_FRAME_IF_DISSIMILAR:
                if len(self.input_face_datas) > num_swapped:
                    return None
            self.num_frames_no_face = 0
            self.last_swapped_frame = temp_frame.copy()
            return temp_frame
        if boop.globals.no_face_action == eNoFaceAction.USE_LAST_SWAPPED:
        ...
        elif boop.globals.no_face_action == eNoFaceAction.USE_ORIGINAL_FRAME:
            return frame
        if boop.globals.no_face_action == eNoFaceAction.SKIP_FRAME:
    ...
    def swap_faces(self, frame, temp_frame):
                        ...
                        if compute_cosine_distance(tf.embedding, face.embedding) <= self.options.face_distance_threshold:
                            if i < len(self.input_face_datas):
                                if use_index:
                                    ...
                                    temp_frame = self.process_face(i, face, temp_frame)
                                num_faces_found += 1
                            if not boop.globals.vr_mode and num_faces_found == num_targetfaces:
                                break
        ...
        if boop.globals.vr_mode and num_faces_found % 2 > 0:
    ...
    def process_face(self,face_index, target_face:Face, frame:Frame):
        from boop.face_util import align_crop
        ...
        if boop.globals.autorotate_faces:
    ...
    # Code for mouth restoration adapted from https://github.com/iVideoGameBoss/iBoopDeepFaceCam
